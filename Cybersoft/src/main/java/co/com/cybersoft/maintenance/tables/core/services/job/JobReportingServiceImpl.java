...
public class JobReportingServiceImpl implements JobReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, JobFilterInfo filter) throws Exception {
		...
		RequestJobPageEvent pageEvent=new RequestJobPageEvent(pageRequest,filter);			

		JobPageEvent requestJobFilter = job.requestJobFilter(pageEvent);
		
		return generateExcel(requestJobFilter.getAllList(), forName, forNameDetails, locale);
	}	
