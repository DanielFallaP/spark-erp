 ...
 *
 */
public class CostingTypeCustomRepoImpl implements CostingTypeCustomRepo {
	...
	public List<CostingType> findAllActiveByCostingType(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM CostingType p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM CostingType p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<CostingType> findAll(Pageable pageable, CostingTypeFilterInfo filt) throws Exception{
		filt.getCostingTypeFilterList().add(filt);
		List<CostingTypeFilterInfo> filters = filt.getCostingTypeFilterList();
		String queryString="SELECT p FROM CostingType p WHERE (((";
		int index=0;
		List<CostingTypeFilterInfo> unionFilters= new ArrayList<CostingTypeFilterInfo>();
		List<CostingTypeFilterInfo> substractFilters= new ArrayList<CostingTypeFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (CostingTypeFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (CostingTypeFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from CostingType p WHERE (":"");
		for (CostingTypeFilterInfo filter : substractFilters) {
		...
		List<CostingType> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getCostingTypeFilterList().remove(filt.getCostingTypeFilterList().size()-1);
		return new PageImpl<CostingType>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(CostingTypeFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
		...
		queryString+="1=1 ";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		if (filter.getCostingType()!=null && !filter.getCostingType().equals(""))queryString+=" AND LOWER(p.costingType) LIKE LOWER('"+filter.getCostingType()+"')";
	...
	private Query buildCriteriaQuery(CostingTypeFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM CostingType p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<CostingType> findAllNoPage(Pageable pageable,
			CostingTypeFilterInfo filt) throws Exception {
		filt.getCostingTypeFilterList().add(filt);
		List<CostingTypeFilterInfo> filters = filt.getCostingTypeFilterList();
		String queryString="SELECT p FROM CostingType p WHERE (((";
		int index=0;
