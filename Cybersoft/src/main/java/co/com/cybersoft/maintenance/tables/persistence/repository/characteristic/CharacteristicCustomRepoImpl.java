 ...
 *
 */
public class CharacteristicCustomRepoImpl implements CharacteristicCustomRepo {
	...
	public List<Characteristic> findAllActiveByCompany(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Characteristic p WHERE p.active = true");
		return query.getResultList();
	}public List<Characteristic> findAllActiveByNameCharacteristic(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Characteristic p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Characteristic p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<Characteristic> findAll(Pageable pageable, CharacteristicFilterInfo filt) throws Exception{
		filt.getCharacteristicFilterList().add(filt);
		List<CharacteristicFilterInfo> filters = filt.getCharacteristicFilterList();
		String queryString="SELECT p FROM Characteristic p WHERE (((";
		int index=0;
		List<CharacteristicFilterInfo> unionFilters= new ArrayList<CharacteristicFilterInfo>();
		List<CharacteristicFilterInfo> substractFilters= new ArrayList<CharacteristicFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (CharacteristicFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (CharacteristicFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from Characteristic p WHERE (":"");
		for (CharacteristicFilterInfo filter : substractFilters) {
		...
		List<Characteristic> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getCharacteristicFilterList().remove(filt.getCharacteristicFilterList().size()-1);
		return new PageImpl<Characteristic>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(CharacteristicFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
		...
		if (filter.getNameCharacteristic()!=null && !filter.getNameCharacteristic().equals(""))queryString+=" AND LOWER(p.nameCharacteristic) LIKE LOWER('"+filter.getNameCharacteristic()+"')";
	...
	private Query buildCriteriaQuery(CharacteristicFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM Characteristic p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<Characteristic> findAllNoPage(Pageable pageable,
			CharacteristicFilterInfo filt) throws Exception {
		filt.getCharacteristicFilterList().add(filt);
		List<CharacteristicFilterInfo> filters = filt.getCharacteristicFilterList();
		String queryString="SELECT p FROM Characteristic p WHERE (((";
		int index=0;
