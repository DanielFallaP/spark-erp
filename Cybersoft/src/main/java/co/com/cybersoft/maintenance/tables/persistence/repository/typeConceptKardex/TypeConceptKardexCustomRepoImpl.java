 ...
 *
 */
public class TypeConceptKardexCustomRepoImpl implements TypeConceptKardexCustomRepo {
	...
	public List<TypeConceptKardex> findAllActiveByTypeConceptKardex(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM TypeConceptKardex p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM TypeConceptKardex p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<TypeConceptKardex> findAll(Pageable pageable, TypeConceptKardexFilterInfo filt) throws Exception{
		filt.getTypeConceptKardexFilterList().add(filt);
		List<TypeConceptKardexFilterInfo> filters = filt.getTypeConceptKardexFilterList();
		String queryString="SELECT p FROM TypeConceptKardex p WHERE (((";
		int index=0;
		List<TypeConceptKardexFilterInfo> unionFilters= new ArrayList<TypeConceptKardexFilterInfo>();
		List<TypeConceptKardexFilterInfo> substractFilters= new ArrayList<TypeConceptKardexFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (TypeConceptKardexFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (TypeConceptKardexFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from TypeConceptKardex p WHERE (":"");
		for (TypeConceptKardexFilterInfo filter : substractFilters) {
		...
		List<TypeConceptKardex> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getTypeConceptKardexFilterList().remove(filt.getTypeConceptKardexFilterList().size()-1);
		return new PageImpl<TypeConceptKardex>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(TypeConceptKardexFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
		...
		queryString+="1=1 ";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		if (filter.getTypeConceptKardex()!=null && !filter.getTypeConceptKardex().equals(""))queryString+=" AND LOWER(p.typeConceptKardex) LIKE LOWER('"+filter.getTypeConceptKardex()+"')";
	...
	private Query buildCriteriaQuery(TypeConceptKardexFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM TypeConceptKardex p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<TypeConceptKardex> findAllNoPage(Pageable pageable,
			TypeConceptKardexFilterInfo filt) throws Exception {
		filt.getTypeConceptKardexFilterList().add(filt);
		List<TypeConceptKardexFilterInfo> filters = filt.getTypeConceptKardexFilterList();
		String queryString="SELECT p FROM TypeConceptKardex p WHERE (((";
		int index=0;
