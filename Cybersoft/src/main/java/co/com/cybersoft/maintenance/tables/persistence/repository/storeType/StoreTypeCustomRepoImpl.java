 ...
 *
 */
public class StoreTypeCustomRepoImpl implements StoreTypeCustomRepo {
	...
	public List<StoreType> findAllActiveByStoreName(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM StoreType p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM StoreType p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<StoreType> findAll(Pageable pageable, StoreTypeFilterInfo filt) throws Exception{
		filt.getStoreTypeFilterList().add(filt);
		List<StoreTypeFilterInfo> filters = filt.getStoreTypeFilterList();
		String queryString="SELECT p FROM StoreType p WHERE (((";
		int index=0;
		List<StoreTypeFilterInfo> unionFilters= new ArrayList<StoreTypeFilterInfo>();
		List<StoreTypeFilterInfo> substractFilters= new ArrayList<StoreTypeFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (StoreTypeFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (StoreTypeFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from StoreType p WHERE (":"");
		for (StoreTypeFilterInfo filter : substractFilters) {
		...
		List<StoreType> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getStoreTypeFilterList().remove(filt.getStoreTypeFilterList().size()-1);
		return new PageImpl<StoreType>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(StoreTypeFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
	...
	private Query buildCriteriaQuery(StoreTypeFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM StoreType p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<StoreType> findAllNoPage(Pageable pageable,
			StoreTypeFilterInfo filt) throws Exception {
		filt.getStoreTypeFilterList().add(filt);
		List<StoreTypeFilterInfo> filters = filt.getStoreTypeFilterList();
		String queryString="SELECT p FROM StoreType p WHERE (((";
		int index=0;
