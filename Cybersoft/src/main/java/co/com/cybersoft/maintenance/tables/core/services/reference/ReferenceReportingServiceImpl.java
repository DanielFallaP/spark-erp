...
public class ReferenceReportingServiceImpl implements ReferenceReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, ReferenceFilterInfo filter) throws Exception {
		...
		RequestReferencePageEvent pageEvent=new RequestReferencePageEvent(pageRequest,filter);			

		ReferencePageEvent requestReferenceFilter = reference.requestReferenceFilter(pageEvent);
		
		return generateExcel(requestReferenceFilter.getAllList(), forName, forNameDetails, locale);
	}	
