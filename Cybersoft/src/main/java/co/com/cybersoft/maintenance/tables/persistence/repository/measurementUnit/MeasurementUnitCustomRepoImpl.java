 ...
 *
 */
public class MeasurementUnitCustomRepoImpl implements MeasurementUnitCustomRepo {
	...
	public List<MeasurementUnit> findAllActiveByCompany(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM MeasurementUnit p WHERE p.active = true");
		return query.getResultList();
	}public List<MeasurementUnit> findAllActiveByNameUnit(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM MeasurementUnit p WHERE p.active = true");
		return query.getResultList();
	}public List<MeasurementUnit> findAllActiveByAbbreviationUnit(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM MeasurementUnit p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM MeasurementUnit p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<MeasurementUnit> findAll(Pageable pageable, MeasurementUnitFilterInfo filt) throws Exception{
		filt.getMeasurementUnitFilterList().add(filt);
		List<MeasurementUnitFilterInfo> filters = filt.getMeasurementUnitFilterList();
		String queryString="SELECT p FROM MeasurementUnit p WHERE (((";
		int index=0;
		List<MeasurementUnitFilterInfo> unionFilters= new ArrayList<MeasurementUnitFilterInfo>();
		List<MeasurementUnitFilterInfo> substractFilters= new ArrayList<MeasurementUnitFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (MeasurementUnitFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (MeasurementUnitFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from MeasurementUnit p WHERE (":"");
		for (MeasurementUnitFilterInfo filter : substractFilters) {
		...
		List<MeasurementUnit> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getMeasurementUnitFilterList().remove(filt.getMeasurementUnitFilterList().size()-1);
		return new PageImpl<MeasurementUnit>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(MeasurementUnitFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
		...
		if (filter.getNameUnit()!=null && !filter.getNameUnit().equals(""))queryString+=" AND LOWER(p.nameUnit) LIKE LOWER('"+filter.getNameUnit()+"')";
		if (filter.getAbbreviationUnit()!=null && !filter.getAbbreviationUnit().equals(""))queryString+=" AND LOWER(p.abbreviationUnit) LIKE LOWER('"+filter.getAbbreviationUnit()+"')";
		if (filter.getTypeUnit()!=null && !filter.getTypeUnit().equals(""))queryString+=" AND p.typeUnit "+filter.getTypeUnit();
		if (filter.getConversionPattern()!=null && !filter.getConversionPattern().equals(""))queryString+=" AND p.conversionPattern "+filter.getConversionPattern();
	...
	private Query buildCriteriaQuery(MeasurementUnitFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM MeasurementUnit p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<MeasurementUnit> findAllNoPage(Pageable pageable,
			MeasurementUnitFilterInfo filt) throws Exception {
		filt.getMeasurementUnitFilterList().add(filt);
		List<MeasurementUnitFilterInfo> filters = filt.getMeasurementUnitFilterList();
		String queryString="SELECT p FROM MeasurementUnit p WHERE (((";
		int index=0;
