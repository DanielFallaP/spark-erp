...
public class CausePendingReportingServiceImpl implements CausePendingReportingService {
	@Autowired
	...
	@Autowired
	private CausePendingService causePending;
	
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, CausePendingFilterInfo filter) throws Exception {
		...
		RequestCausePendingPageEvent pageEvent=new RequestCausePendingPageEvent(pageRequest,filter);			

		CausePendingPageEvent requestCausePendingFilter = causePending.requestCausePendingFilter(pageEvent);
		
		return generateExcel(requestCausePendingFilter.getAllList(), forName, forNameDetails, locale);
	}	
