...
public class AccountantReportingServiceImpl implements AccountantReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, AccountantFilterInfo filter) throws Exception {
		...
		RequestAccountantPageEvent pageEvent=new RequestAccountantPageEvent(pageRequest,filter);			

		AccountantPageEvent requestAccountantFilter = accountant.requestAccountantFilter(pageEvent);
		
		return generateExcel(requestAccountantFilter.getAllList(), forName, forNameDetails, locale);
	}	
