...
public class StoreTypeReportingServiceImpl implements StoreTypeReportingService {
	@Autowired
	...
	@Autowired
	private StoreTypeService storeType;
	
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, StoreTypeFilterInfo filter) throws Exception {
		...
		RequestStoreTypePageEvent pageEvent=new RequestStoreTypePageEvent(pageRequest,filter);			

		StoreTypePageEvent requestStoreTypeFilter = storeType.requestStoreTypeFilter(pageEvent);
		
		return generateExcel(requestStoreTypeFilter.getAllList(), forName, forNameDetails, locale);
	}	
