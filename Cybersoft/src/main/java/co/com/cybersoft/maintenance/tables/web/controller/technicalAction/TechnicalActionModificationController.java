...
/**
 * Controller class for TechnicalAction
 * @author Cybersystems 2014. All rights reserved.
 *
 */
@Controller
@RequestMapping("/maintenance/technicalAction/modifyTechnicalAction/{id}")
public class TechnicalActionModificationController {
	...
	public String modification(Model model, HttpServletRequest request){
		...
		return "/maintenance/technicalAction/modifyTechnicalAction";
		
	}
	
	@RequestMapping(method=RequestMethod.POST)
	@Transactional(readOnly=false)
	public String modifyTechnicalAction(Model model, @PathVariable("id") Long id, @Valid @ModelAttribute("technicalActionInfo") TechnicalActionInfo technicalActionInfo, HttpServletRequest request) throws Exception {
		...
		//technicalActionDetails.set_companyId(((TechnicalActionDetails)request.getSession().getAttribute("_loggedInUser")).getCompanyId());

		model.addAttribute("_loggedInUser", request.getSession().getAttribute("_loggedInUser"));
		
		
		request.getSession().setAttribute("technicalActionInfo", technicalActionInfo);
		technicalActionService.modifyTechnicalAction(new TechnicalActionModificationEvent(technicalActionDetails));
		return "redirect:/maintenance/technicalAction/searchTechnicalAction";
	}
	...
	private TechnicalActionInfo getTechnicalActionInfo(@PathVariable("id") Long id, HttpServletRequest request) throws Exception {
		LOG.debug("Retrieving the technicalAction "+id);
		
		TechnicalActionInfo technicalActionInfo = new TechnicalActionInfo();
			
		TechnicalActionDetailsEvent requestTechnicalActionDetails = technicalActionService.requestTechnicalActionDetails(new RequestTechnicalActionDetailsEvent(id));
		...
		BeanUtils.copyProperties(requestTechnicalActionDetails.getTechnicalActionDetails(), technicalActionInfo);
		request.getSession().setAttribute("technicalActionInfo", technicalActionInfo);
	...
	public ModelAndView constraintError(HttpServletRequest req, Exception exception){
		...
		if (exception instanceof BindException){
			...
			modelAndView.setViewName("/maintenance/technicalAction/modifyTechnicalAction");
		}
		else{
			modelAndView.addObject("technicalActionInfo", req.getSession().getAttribute("technicalActionInfo"));
			modelAndView.addObject("technicalActionModificationException",true);
			modelAndView.setViewName("/maintenance/technicalAction/modifyTechnicalAction");
		}
		return modelAndView;
	}
