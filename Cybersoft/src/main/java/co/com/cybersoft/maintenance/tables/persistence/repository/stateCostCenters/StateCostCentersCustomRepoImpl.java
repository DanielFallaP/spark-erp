 ...
 *
 */
public class StateCostCentersCustomRepoImpl implements StateCostCentersCustomRepo {
	...
	public List<StateCostCenters> findAllActiveByStateCostCenters(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM StateCostCenters p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM StateCostCenters p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<StateCostCenters> findAll(Pageable pageable, StateCostCentersFilterInfo filt) throws Exception{
		filt.getStateCostCentersFilterList().add(filt);
		List<StateCostCentersFilterInfo> filters = filt.getStateCostCentersFilterList();
		String queryString="SELECT p FROM StateCostCenters p WHERE (((";
		int index=0;
		List<StateCostCentersFilterInfo> unionFilters= new ArrayList<StateCostCentersFilterInfo>();
		List<StateCostCentersFilterInfo> substractFilters= new ArrayList<StateCostCentersFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (StateCostCentersFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (StateCostCentersFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from StateCostCenters p WHERE (":"");
		for (StateCostCentersFilterInfo filter : substractFilters) {
		...
		List<StateCostCenters> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getStateCostCentersFilterList().remove(filt.getStateCostCentersFilterList().size()-1);
		return new PageImpl<StateCostCenters>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(StateCostCentersFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
		...
		queryString+="1=1 ";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		if (filter.getStateCostCenters()!=null && !filter.getStateCostCenters().equals(""))queryString+=" AND LOWER(p.stateCostCenters) LIKE LOWER('"+filter.getStateCostCenters()+"')";
	...
	private Query buildCriteriaQuery(StateCostCentersFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM StateCostCenters p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<StateCostCenters> findAllNoPage(Pageable pageable,
			StateCostCentersFilterInfo filt) throws Exception {
		filt.getStateCostCentersFilterList().add(filt);
		List<StateCostCentersFilterInfo> filters = filt.getStateCostCentersFilterList();
		String queryString="SELECT p FROM StateCostCenters p WHERE (((";
		int index=0;
