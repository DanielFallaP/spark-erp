...
public class ContractReportingServiceImpl implements ContractReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, ContractFilterInfo filter) throws Exception {
		...
		RequestContractPageEvent pageEvent=new RequestContractPageEvent(pageRequest,filter);			

		ContractPageEvent requestContractFilter = contract.requestContractFilter(pageEvent);
		
		return generateExcel(requestContractFilter.getAllList(), forName, forNameDetails, locale);
	}	
