...
public class StateReportingServiceImpl implements StateReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, StateFilterInfo filter) throws Exception {
		...
		RequestStatePageEvent pageEvent=new RequestStatePageEvent(pageRequest,filter);			

		StatePageEvent requestStateFilter = state.requestStateFilter(pageEvent);
		
		return generateExcel(requestStateFilter.getAllList(), forName, forNameDetails, locale);
	}	
