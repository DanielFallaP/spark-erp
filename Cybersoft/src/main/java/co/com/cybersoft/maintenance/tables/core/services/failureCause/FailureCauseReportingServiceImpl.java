...
public class FailureCauseReportingServiceImpl implements FailureCauseReportingService {
	@Autowired
	...
	@Autowired
	private FailureCauseService failureCause;
	
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, FailureCauseFilterInfo filter) throws Exception {
		...
		RequestFailureCausePageEvent pageEvent=new RequestFailureCausePageEvent(pageRequest,filter);			

		FailureCausePageEvent requestFailureCauseFilter = failureCause.requestFailureCauseFilter(pageEvent);
		
		return generateExcel(requestFailureCauseFilter.getAllList(), forName, forNameDetails, locale);
	}	
