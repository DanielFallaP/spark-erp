 ...
 *
 */
public class ReferenceCustomRepoImpl implements ReferenceCustomRepo {
	...
	public List<Reference> findAllActiveByCompany(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Reference p WHERE p.active = true");
		return query.getResultList();
	}public List<Reference> findAllActiveByCodeReference(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Reference p WHERE p.active = true");
		return query.getResultList();
	}public List<Reference> findAllActiveByNameReference(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Reference p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Reference p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<Reference> findAll(Pageable pageable, ReferenceFilterInfo filt) throws Exception{
		filt.getReferenceFilterList().add(filt);
		List<ReferenceFilterInfo> filters = filt.getReferenceFilterList();
		String queryString="SELECT p FROM Reference p WHERE (((";
		int index=0;
		List<ReferenceFilterInfo> unionFilters= new ArrayList<ReferenceFilterInfo>();
		List<ReferenceFilterInfo> substractFilters= new ArrayList<ReferenceFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (ReferenceFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (ReferenceFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from Reference p WHERE (":"");
		for (ReferenceFilterInfo filter : substractFilters) {
		...
		List<Reference> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getReferenceFilterList().remove(filt.getReferenceFilterList().size()-1);
		return new PageImpl<Reference>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(ReferenceFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
		...
		if (filter.getCodeReference()!=null && !filter.getCodeReference().equals(""))queryString+=" AND LOWER(p.codeReference) LIKE LOWER('"+filter.getCodeReference()+"')";
		if (filter.getNameReference()!=null && !filter.getNameReference().equals(""))queryString+=" AND LOWER(p.nameReference) LIKE LOWER('"+filter.getNameReference()+"')";
	...
	private Query buildCriteriaQuery(ReferenceFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM Reference p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<Reference> findAllNoPage(Pageable pageable,
			ReferenceFilterInfo filt) throws Exception {
		filt.getReferenceFilterList().add(filt);
		List<ReferenceFilterInfo> filters = filt.getReferenceFilterList();
		String queryString="SELECT p FROM Reference p WHERE (((";
		int index=0;
