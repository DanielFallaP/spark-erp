 ...
 *
 */
public class TypeMaintenanceCustomRepoImpl implements TypeMaintenanceCustomRepo {
	...
	public List<TypeMaintenance> findAllActiveByCompany(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM TypeMaintenance p WHERE p.active = true");
		return query.getResultList();
	}public List<TypeMaintenance> findAllActiveByNameTypeMaintenance(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM TypeMaintenance p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM TypeMaintenance p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<TypeMaintenance> findAll(Pageable pageable, TypeMaintenanceFilterInfo filt) throws Exception{
		filt.getTypeMaintenanceFilterList().add(filt);
		List<TypeMaintenanceFilterInfo> filters = filt.getTypeMaintenanceFilterList();
		String queryString="SELECT p FROM TypeMaintenance p WHERE (((";
		int index=0;
		List<TypeMaintenanceFilterInfo> unionFilters= new ArrayList<TypeMaintenanceFilterInfo>();
		List<TypeMaintenanceFilterInfo> substractFilters= new ArrayList<TypeMaintenanceFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (TypeMaintenanceFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (TypeMaintenanceFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from TypeMaintenance p WHERE (":"");
		for (TypeMaintenanceFilterInfo filter : substractFilters) {
		...
		List<TypeMaintenance> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getTypeMaintenanceFilterList().remove(filt.getTypeMaintenanceFilterList().size()-1);
		return new PageImpl<TypeMaintenance>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(TypeMaintenanceFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
		...
		if (filter.getNameTypeMaintenance()!=null && !filter.getNameTypeMaintenance().equals(""))queryString+=" AND LOWER(p.nameTypeMaintenance) LIKE LOWER('"+filter.getNameTypeMaintenance()+"')";
	...
	private Query buildCriteriaQuery(TypeMaintenanceFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM TypeMaintenance p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<TypeMaintenance> findAllNoPage(Pageable pageable,
			TypeMaintenanceFilterInfo filt) throws Exception {
		filt.getTypeMaintenanceFilterList().add(filt);
		List<TypeMaintenanceFilterInfo> filters = filt.getTypeMaintenanceFilterList();
		String queryString="SELECT p FROM TypeMaintenance p WHERE (((";
		int index=0;
