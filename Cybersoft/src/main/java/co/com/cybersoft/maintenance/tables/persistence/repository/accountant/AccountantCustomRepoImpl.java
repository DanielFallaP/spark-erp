 ...
 *
 */
public class AccountantCustomRepoImpl implements AccountantCustomRepo {
	...
	public List<Accountant> findAllActiveByCompany(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Accountant p WHERE p.active = true");
		return query.getResultList();
	}public List<Accountant> findAllActiveByNameAccountant(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Accountant p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Accountant p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<Accountant> findAll(Pageable pageable, AccountantFilterInfo filt) throws Exception{
		filt.getAccountantFilterList().add(filt);
		List<AccountantFilterInfo> filters = filt.getAccountantFilterList();
		String queryString="SELECT p FROM Accountant p WHERE (((";
		int index=0;
		List<AccountantFilterInfo> unionFilters= new ArrayList<AccountantFilterInfo>();
		List<AccountantFilterInfo> substractFilters= new ArrayList<AccountantFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (AccountantFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (AccountantFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from Accountant p WHERE (":"");
		for (AccountantFilterInfo filter : substractFilters) {
		...
		List<Accountant> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getAccountantFilterList().remove(filt.getAccountantFilterList().size()-1);
		return new PageImpl<Accountant>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(AccountantFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
		...
		if (filter.getNameAccountant()!=null && !filter.getNameAccountant().equals(""))queryString+=" AND LOWER(p.nameAccountant) LIKE LOWER('"+filter.getNameAccountant()+"')";
		if (filter.getNumber()!=null && !filter.getNumber().equals(""))queryString+=" AND p.number "+filter.getNumber();
	...
	private Query buildCriteriaQuery(AccountantFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM Accountant p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<Accountant> findAllNoPage(Pageable pageable,
			AccountantFilterInfo filt) throws Exception {
		filt.getAccountantFilterList().add(filt);
		List<AccountantFilterInfo> filters = filt.getAccountantFilterList();
		String queryString="SELECT p FROM Accountant p WHERE (((";
		int index=0;
