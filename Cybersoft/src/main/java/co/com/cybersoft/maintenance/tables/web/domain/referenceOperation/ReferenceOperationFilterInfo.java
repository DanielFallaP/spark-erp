...
public class ReferenceOperationFilterInfo {
	...
	private String numOrderRefenceOperation;


	public String getNumOrderRefenceOperation() {
		return numOrderRefenceOperation;	
	}
		
	public void setNumOrderRefenceOperation(String numOrderRefenceOperation) {
	...
	private List<ReferenceOperationFilterInfo> referenceOperationFilterList=new ArrayList<ReferenceOperationFilterInfo>();
	
	public List<ReferenceOperationFilterInfo> getReferenceOperationFilterList() {
		return referenceOperationFilterList;
	}

	public void setReferenceOperationFilterList(
			List<ReferenceOperationFilterInfo> referenceOperationFilterList) {
		this.referenceOperationFilterList = referenceOperationFilterList;
	}
	...
	public String getFfilterAsText() {
		ffilterAsText="";
		int index=0;
		for (ReferenceOperationFilterInfo fil : referenceOperationFilterList) {
			ffilterAsText+=(index!=0?(fil.getFffilterNature().equals(CyberUtils.filterUnion)?" UNION ":" MINUS "):"");
			if (fil.getReference()!=null && !fil.getReference().equals(""))ffilterAsText+=fil.getReference()+" ";
			if (fil.getOperation()!=null && !fil.getOperation().equals(""))ffilterAsText+=fil.getOperation()+" ";
			if (fil.getNumOrderRefenceOperation()!=null && !fil.getNumOrderRefenceOperation().equals(""))ffilterAsText+=fil.getNumOrderRefenceOperation()+" ";
		...
		if (reference!=null && !reference.equals(""))ffilterAsText+=reference+" ";if (operation!=null && !operation.equals(""))ffilterAsText+=operation+" ";if (numOrderRefenceOperation!=null && !numOrderRefenceOperation.equals(""))ffilterAsText+=numOrderRefenceOperation+" ";if (active!=null && !active.equals(""))ffilterAsText+=active+" ";
