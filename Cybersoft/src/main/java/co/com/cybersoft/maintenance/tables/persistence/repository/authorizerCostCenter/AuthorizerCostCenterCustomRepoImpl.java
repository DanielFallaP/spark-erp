 ...
 *
 */
public class AuthorizerCostCenterCustomRepoImpl implements AuthorizerCostCenterCustomRepo {
	...
	public List<AuthorizerCostCenter> findAllActiveByCostCenter(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM AuthorizerCostCenter p WHERE p.active = true");
		return query.getResultList();
	}public List<AuthorizerCostCenter> findAllActiveByResponsible(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM AuthorizerCostCenter p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM AuthorizerCostCenter p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<AuthorizerCostCenter> findAll(Pageable pageable, AuthorizerCostCenterFilterInfo filt) throws Exception{
		filt.getAuthorizerCostCenterFilterList().add(filt);
		List<AuthorizerCostCenterFilterInfo> filters = filt.getAuthorizerCostCenterFilterList();
		String queryString="SELECT p FROM AuthorizerCostCenter p WHERE (((";
		int index=0;
		List<AuthorizerCostCenterFilterInfo> unionFilters= new ArrayList<AuthorizerCostCenterFilterInfo>();
		List<AuthorizerCostCenterFilterInfo> substractFilters= new ArrayList<AuthorizerCostCenterFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (AuthorizerCostCenterFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (AuthorizerCostCenterFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from AuthorizerCostCenter p WHERE (":"");
		for (AuthorizerCostCenterFilterInfo filter : substractFilters) {
		...
		List<AuthorizerCostCenter> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getAuthorizerCostCenterFilterList().remove(filt.getAuthorizerCostCenterFilterList().size()-1);
		return new PageImpl<AuthorizerCostCenter>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(AuthorizerCostCenterFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
	...
	private Query buildCriteriaQuery(AuthorizerCostCenterFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM AuthorizerCostCenter p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<AuthorizerCostCenter> findAllNoPage(Pageable pageable,
			AuthorizerCostCenterFilterInfo filt) throws Exception {
		filt.getAuthorizerCostCenterFilterList().add(filt);
		List<AuthorizerCostCenterFilterInfo> filters = filt.getAuthorizerCostCenterFilterList();
		String queryString="SELECT p FROM AuthorizerCostCenter p WHERE (((";
		int index=0;
