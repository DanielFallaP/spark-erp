...
public class CompanyReportingServiceImpl implements CompanyReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, CompanyFilterInfo filter) throws Exception {
		...
		RequestCompanyPageEvent pageEvent=new RequestCompanyPageEvent(pageRequest,filter);			

		CompanyPageEvent requestCompanyFilter = company.requestCompanyFilter(pageEvent);
		
		return generateExcel(requestCompanyFilter.getAllList(), forName, forNameDetails, locale);
	}	
