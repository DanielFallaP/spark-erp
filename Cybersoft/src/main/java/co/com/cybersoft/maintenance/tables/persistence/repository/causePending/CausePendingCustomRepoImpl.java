 ...
 *
 */
public class CausePendingCustomRepoImpl implements CausePendingCustomRepo {
	...
	public List<CausePending> findAllActiveByCompany(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM CausePending p WHERE p.active = true");
		return query.getResultList();
	}public List<CausePending> findAllActiveByNameCausePending(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM CausePending p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM CausePending p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<CausePending> findAll(Pageable pageable, CausePendingFilterInfo filt) throws Exception{
		filt.getCausePendingFilterList().add(filt);
		List<CausePendingFilterInfo> filters = filt.getCausePendingFilterList();
		String queryString="SELECT p FROM CausePending p WHERE (((";
		int index=0;
		List<CausePendingFilterInfo> unionFilters= new ArrayList<CausePendingFilterInfo>();
		List<CausePendingFilterInfo> substractFilters= new ArrayList<CausePendingFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (CausePendingFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (CausePendingFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from CausePending p WHERE (":"");
		for (CausePendingFilterInfo filter : substractFilters) {
		...
		List<CausePending> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getCausePendingFilterList().remove(filt.getCausePendingFilterList().size()-1);
		return new PageImpl<CausePending>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(CausePendingFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
		...
		if (filter.getNameCausePending()!=null && !filter.getNameCausePending().equals(""))queryString+=" AND LOWER(p.nameCausePending) LIKE LOWER('"+filter.getNameCausePending()+"')";
	...
	private Query buildCriteriaQuery(CausePendingFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM CausePending p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<CausePending> findAllNoPage(Pageable pageable,
			CausePendingFilterInfo filt) throws Exception {
		filt.getCausePendingFilterList().add(filt);
		List<CausePendingFilterInfo> filters = filt.getCausePendingFilterList();
		String queryString="SELECT p FROM CausePending p WHERE (((";
		int index=0;
