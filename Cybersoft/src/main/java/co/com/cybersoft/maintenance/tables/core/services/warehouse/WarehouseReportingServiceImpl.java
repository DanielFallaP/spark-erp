...
public class WarehouseReportingServiceImpl implements WarehouseReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, WarehouseFilterInfo filter) throws Exception {
		...
		RequestWarehousePageEvent pageEvent=new RequestWarehousePageEvent(pageRequest,filter);			

		WarehousePageEvent requestWarehouseFilter = warehouse.requestWarehouseFilter(pageEvent);
		
		return generateExcel(requestWarehouseFilter.getAllList(), forName, forNameDetails, locale);
	}	
