...
public class EffectFailReportingServiceImpl implements EffectFailReportingService {
	@Autowired
	...
	@Autowired
	private EffectFailService effectFail;
	
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, EffectFailFilterInfo filter) throws Exception {
		...
		RequestEffectFailPageEvent pageEvent=new RequestEffectFailPageEvent(pageRequest,filter);			

		EffectFailPageEvent requestEffectFailFilter = effectFail.requestEffectFailFilter(pageEvent);
		
		return generateExcel(requestEffectFailFilter.getAllList(), forName, forNameDetails, locale);
	}	
