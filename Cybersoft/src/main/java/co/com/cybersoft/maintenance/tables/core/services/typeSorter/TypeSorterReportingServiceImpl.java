...
public class TypeSorterReportingServiceImpl implements TypeSorterReportingService {
	@Autowired
	...
	@Autowired
	private TypeSorterService typeSorter;
	
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, TypeSorterFilterInfo filter) throws Exception {
		...
		RequestTypeSorterPageEvent pageEvent=new RequestTypeSorterPageEvent(pageRequest,filter);			

		TypeSorterPageEvent requestTypeSorterFilter = typeSorter.requestTypeSorterFilter(pageEvent);
		
		return generateExcel(requestTypeSorterFilter.getAllList(), forName, forNameDetails, locale);
	}	
