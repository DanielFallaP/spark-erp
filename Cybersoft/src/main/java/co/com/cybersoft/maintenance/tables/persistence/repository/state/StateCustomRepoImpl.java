...
public class StateCustomRepoImpl implements StateCustomRepo {
	...
	public List<State> findAllActiveByCompany(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM State p WHERE p.active = true");
		return query.getResultList();
	}public List<State> findAllActiveByNameState(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM State p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM State p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<State> findAll(Pageable pageable, StateFilterInfo filt) throws Exception{
		filt.getStateFilterList().add(filt);
		List<StateFilterInfo> filters = filt.getStateFilterList();
		String queryString="SELECT p FROM State p WHERE (((";
		int index=0;
		List<StateFilterInfo> unionFilters= new ArrayList<StateFilterInfo>();
		List<StateFilterInfo> substractFilters= new ArrayList<StateFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (StateFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (StateFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from State p WHERE (":"");
		for (StateFilterInfo filter : substractFilters) {
		...
		List<State> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getStateFilterList().remove(filt.getStateFilterList().size()-1);
		return new PageImpl<State>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(StateFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
		...
		if (filter.getNameState()!=null && !filter.getNameState().equals(""))queryString+=" AND LOWER(p.nameState) LIKE LOWER('"+filter.getNameState()+"')";
		if (filter.getTypeState()!=null && !filter.getTypeState().equals(""))queryString+=" AND p.typeState "+filter.getTypeState();
	...
	private Query buildCriteriaQuery(StateFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM State p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<State> findAllNoPage(Pageable pageable,
			StateFilterInfo filt) throws Exception {
		filt.getStateFilterList().add(filt);
		List<StateFilterInfo> filters = filt.getStateFilterList();
		String queryString="SELECT p FROM State p WHERE (((";
		int index=0;
