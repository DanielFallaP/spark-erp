...
public class SorterReportingServiceImpl implements SorterReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, SorterFilterInfo filter) throws Exception {
		...
		RequestSorterPageEvent pageEvent=new RequestSorterPageEvent(pageRequest,filter);			

		SorterPageEvent requestSorterFilter = sorter.requestSorterFilter(pageEvent);
		
		return generateExcel(requestSorterFilter.getAllList(), forName, forNameDetails, locale);
	}	
