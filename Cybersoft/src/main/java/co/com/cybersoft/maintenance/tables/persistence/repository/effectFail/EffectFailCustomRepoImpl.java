 ...
 *
 */
public class EffectFailCustomRepoImpl implements EffectFailCustomRepo {
	...
	public List<EffectFail> findAllActiveByCompany(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM EffectFail p WHERE p.active = true");
		return query.getResultList();
	}public List<EffectFail> findAllActiveByEffectFailName(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM EffectFail p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM EffectFail p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<EffectFail> findAll(Pageable pageable, EffectFailFilterInfo filt) throws Exception{
		filt.getEffectFailFilterList().add(filt);
		List<EffectFailFilterInfo> filters = filt.getEffectFailFilterList();
		String queryString="SELECT p FROM EffectFail p WHERE (((";
		int index=0;
		List<EffectFailFilterInfo> unionFilters= new ArrayList<EffectFailFilterInfo>();
		List<EffectFailFilterInfo> substractFilters= new ArrayList<EffectFailFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (EffectFailFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (EffectFailFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from EffectFail p WHERE (":"");
		for (EffectFailFilterInfo filter : substractFilters) {
		...
		List<EffectFail> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getEffectFailFilterList().remove(filt.getEffectFailFilterList().size()-1);
		return new PageImpl<EffectFail>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(EffectFailFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
		...
		if (filter.getEffectFailName()!=null && !filter.getEffectFailName().equals(""))queryString+=" AND LOWER(p.effectFailName) LIKE LOWER('"+filter.getEffectFailName()+"')";
	...
	private Query buildCriteriaQuery(EffectFailFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM EffectFail p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<EffectFail> findAllNoPage(Pageable pageable,
			EffectFailFilterInfo filt) throws Exception {
		filt.getEffectFailFilterList().add(filt);
		List<EffectFailFilterInfo> filters = filt.getEffectFailFilterList();
		String queryString="SELECT p FROM EffectFail p WHERE (((";
		int index=0;
