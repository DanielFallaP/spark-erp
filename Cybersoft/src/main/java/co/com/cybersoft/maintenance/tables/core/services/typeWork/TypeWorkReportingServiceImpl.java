...
public class TypeWorkReportingServiceImpl implements TypeWorkReportingService {
	@Autowired
	...
	@Autowired
	private TypeWorkService typeWork;
	
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, TypeWorkFilterInfo filter) throws Exception {
		...
		RequestTypeWorkPageEvent pageEvent=new RequestTypeWorkPageEvent(pageRequest,filter);			

		TypeWorkPageEvent requestTypeWorkFilter = typeWork.requestTypeWorkFilter(pageEvent);
		
		return generateExcel(requestTypeWorkFilter.getAllList(), forName, forNameDetails, locale);
	}	
