...
public class CostingTypeReportingServiceImpl implements CostingTypeReportingService {
	@Autowired
	...
	@Autowired
	private CostingTypeService costingType;
	
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, CostingTypeFilterInfo filter) throws Exception {
		...
		RequestCostingTypePageEvent pageEvent=new RequestCostingTypePageEvent(pageRequest,filter);			

		CostingTypePageEvent requestCostingTypeFilter = costingType.requestCostingTypeFilter(pageEvent);
		
		return generateExcel(requestCostingTypeFilter.getAllList(), forName, forNameDetails, locale);
	}	
