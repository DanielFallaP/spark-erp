...
public class OperationReportingServiceImpl implements OperationReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, OperationFilterInfo filter) throws Exception {
		...
		RequestOperationPageEvent pageEvent=new RequestOperationPageEvent(pageRequest,filter);			

		OperationPageEvent requestOperationFilter = operation.requestOperationFilter(pageEvent);
		
		return generateExcel(requestOperationFilter.getAllList(), forName, forNameDetails, locale);
	}	
