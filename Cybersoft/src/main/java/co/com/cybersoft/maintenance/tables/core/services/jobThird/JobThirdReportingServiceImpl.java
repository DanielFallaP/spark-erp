...
public class JobThirdReportingServiceImpl implements JobThirdReportingService {
	@Autowired
	...
	@Autowired
	private JobThirdService jobThird;
	
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, JobThirdFilterInfo filter) throws Exception {
		...
		RequestJobThirdPageEvent pageEvent=new RequestJobThirdPageEvent(pageRequest,filter);			

		JobThirdPageEvent requestJobThirdFilter = jobThird.requestJobThirdFilter(pageEvent);
		
		return generateExcel(requestJobThirdFilter.getAllList(), forName, forNameDetails, locale);
	}	
