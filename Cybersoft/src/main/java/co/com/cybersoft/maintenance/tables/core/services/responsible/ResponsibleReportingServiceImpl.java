...
public class ResponsibleReportingServiceImpl implements ResponsibleReportingService {
	@Autowired
	...
	@Autowired
	private ResponsibleService responsible;
	
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, ResponsibleFilterInfo filter) throws Exception {
		...
		RequestResponsiblePageEvent pageEvent=new RequestResponsiblePageEvent(pageRequest,filter);			

		ResponsiblePageEvent requestResponsibleFilter = responsible.requestResponsibleFilter(pageEvent);
		
		return generateExcel(requestResponsibleFilter.getAllList(), forName, forNameDetails, locale);
	}	
