...
public class ContinentReportingServiceImpl implements ContinentReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, ContinentFilterInfo filter) throws Exception {
		...
		RequestContinentPageEvent pageEvent=new RequestContinentPageEvent(pageRequest,filter);			

		ContinentPageEvent requestContinentFilter = continent.requestContinentFilter(pageEvent);
		
		return generateExcel(requestContinentFilter.getAllList(), forName, forNameDetails, locale);
	}	
