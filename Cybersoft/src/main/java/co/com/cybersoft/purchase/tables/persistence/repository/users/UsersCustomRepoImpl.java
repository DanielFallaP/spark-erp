...
public class UsersCustomRepoImpl implements UsersCustomRepo {
	...
	public List<Users> findAllActiveByLogin(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Users p WHERE p.active = true");
		return query.getResultList();
	}public List<Users> findAllActiveByPassword(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Users p WHERE p.active = true");
		return query.getResultList();
	}public List<Users> findAllActiveByRole(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Users p WHERE p.active = true");
		return query.getResultList();
	}public List<Users> findAllActiveByCompany(EmbeddedField... fields) throws Exception {
		Query query = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Users p WHERE p.active = true");
		return query.getResultList();
	}
	
	public Long getTotalCount() throws Exception{
		Query query2 = emFactory.getObject().createEntityManager().createQuery("SELECT p FROM Users p WHERE p.active = true");
		return (long) query2.getResultList().size();
	}

	public Page<Users> findAll(Pageable pageable, UsersFilterInfo filt) throws Exception{
		filt.getUsersFilterList().add(filt);
		List<UsersFilterInfo> filters = filt.getUsersFilterList();
		String queryString="SELECT p FROM Users p WHERE (((";
		int index=0;
		List<UsersFilterInfo> unionFilters= new ArrayList<UsersFilterInfo>();
		List<UsersFilterInfo> substractFilters= new ArrayList<UsersFilterInfo>();
		unionFilters.add(filters.get(0));
		
		for (UsersFilterInfo filter : filters) {
			if (index!=0)
		...
		index=0;
		for (UsersFilterInfo filter : unionFilters) {
		...
		int index2=index;
		index=0;
		queryString+=(substractFilters.size()>0?" AND p.id NOT IN (select p from Users p WHERE (":"");
		for (UsersFilterInfo filter : substractFilters) {
		...
		List<Users> list = query.setFirstResult(pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
		
		filt.getUsersFilterList().remove(filt.getUsersFilterList().size()-1);
		return new PageImpl<Users>(list, pageable, count);
	}
	
	private String buildCriteriaQuery(UsersFilterInfo filter, Pageable pageable, int index, int offset, String nature)throws Exception{
		String queryString="";
		...
		queryString+="1=1 ";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		if (filter.getLogin()!=null && !filter.getLogin().equals(""))queryString+=" AND LOWER(p.login) LIKE LOWER('"+filter.getLogin()+"')";
		if (filter.getPassword()!=null && !filter.getPassword().equals(""))queryString+=" AND LOWER(p.password) LIKE LOWER('"+filter.getPassword()+"')";
		if (filter.getRole()!=null && !filter.getRole().equals(""))queryString+=" AND LOWER(p.role) LIKE LOWER('"+filter.getRole()+"')";
	...
	private Query buildCriteriaQuery(UsersFilterInfo filter, Pageable pageable)throws Exception{
		String queryString="SELECT p FROM Users p WHERE 1=1";
		DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	...
	public List<Users> findAllNoPage(Pageable pageable,
			UsersFilterInfo filt) throws Exception {
		filt.getUsersFilterList().add(filt);
		List<UsersFilterInfo> filters = filt.getUsersFilterList();
		String queryString="SELECT p FROM Users p WHERE (((";
		int index=0;
