...
public class CountryReportingServiceImpl implements CountryReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, CountryFilterInfo filter) throws Exception {
		...
		RequestCountryPageEvent pageEvent=new RequestCountryPageEvent(pageRequest,filter);			

		CountryPageEvent requestCountryFilter = country.requestCountryFilter(pageEvent);
		
		return generateExcel(requestCountryFilter.getAllList(), forName, forNameDetails, locale);
	}	
