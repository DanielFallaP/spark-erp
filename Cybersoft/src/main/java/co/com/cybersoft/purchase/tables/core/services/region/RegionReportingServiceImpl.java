...
public class RegionReportingServiceImpl implements RegionReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, RegionFilterInfo filter) throws Exception {
		...
		RequestRegionPageEvent pageEvent=new RequestRegionPageEvent(pageRequest,filter);			

		RegionPageEvent requestRegionFilter = region.requestRegionFilter(pageEvent);
		
		return generateExcel(requestRegionFilter.getAllList(), forName, forNameDetails, locale);
	}	
