...
public class CurrencyReportingServiceImpl implements CurrencyReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, CurrencyFilterInfo filter) throws Exception {
		...
		RequestCurrencyPageEvent pageEvent=new RequestCurrencyPageEvent(pageRequest,filter);			

		CurrencyPageEvent requestCurrencyFilter = currency.requestCurrencyFilter(pageEvent);
		
		return generateExcel(requestCurrencyFilter.getAllList(), forName, forNameDetails, locale);
	}	
