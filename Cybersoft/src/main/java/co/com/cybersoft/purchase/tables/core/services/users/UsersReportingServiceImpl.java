...
public class UsersReportingServiceImpl implements UsersReportingService {
	@Autowired
	...
	public ExcelReportResponse toExcel(String className, String detailsClassName, Locale locale, UsersFilterInfo filter) throws Exception {
		...
		RequestUsersPageEvent pageEvent=new RequestUsersPageEvent(pageRequest,filter);			

		UsersPageEvent requestUsersFilter = users.requestUsersFilter(pageEvent);
		
		return generateExcel(requestUsersFilter.getAllList(), forName, forNameDetails, locale);
	}	
